...
class Movie extends Video
{
    ...
    {
        if (!isset($this->binary_conditions['is_movie'])) $this->setBinary_conditions(array("is_movie"=>1));
    ...
    {
        $this->setBinary_conditions(array("is_movie"=>1));
    ...
    public function generateViewList(array $movie_array)
    {
        $string = "";
        foreach ($movie_array as $movie) {
            $videoItem = PT_GetVideoByID(intval($movie['id']), 0, 0, 2);
...
/*
 * movie_release => int: release year
 * country => int ? check `langs`
 * categories => 255char String
 * Javayez =>
 * rating => int : 1->10
 *
 */