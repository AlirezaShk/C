...
$featured = $db->where('is_movie', 1)->get($table,$limit,array('video_id','user_id'));
...
if (!empty($_GET['category']) && $_GET['category'] != 'other') {
    $db->where('category_id', PT_Secure($_GET['category']));
}
...
foreach ($pt->movies_categories as $key => $category) {
    $response_data['genres'][] = ['name'=>$category, 'id'=>$key];
}
