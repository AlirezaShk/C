...
//    $trending_data = $db->where('is_movie', 1, '!=')->where('is_movie', 3, '!=')->where('is_movie', 2, '!=')->where('time', time() - (TRENDING_VIDEO_DAY_PERIOD * 24 * 3600), '>')->orderBy('views', 'DESC')->get(T_VIDEOS);
    $trending = $db->where('privacy', 0)->where('is_movie',0)->orderBy('id', 'DESC')->get(T_VIDEOS);
...
//    $trending_data = $db->where('privacy', 0)->where('is_movie',0)->orderBy('id', 'DESC')->get(T_VIDEOS);
    $trending = $db->where('privacy', 0)->where('is_movie',0)->where('time', time() - (TRENDING_VIDEO_DAY_PERIOD * 30 * 24 * 3600), '>')->orderBy('id', 'DESC')->get(T_VIDEOS);
    $trending = sortVideosByTrendScore($trending);
}
$cnt = 0;
...
if (!empty($pro_users)){
    ...
    $top = $db->where('is_movie', 0)->get(T_VIDEOS, $limit,array('video_id','user_id'));
}
...
if (!empty($pro_users)) {
    ...
    $db->where('privacy', 0);
    $db->orderBy('id', 'DESC');
    $latest = $db->where('is_movie', 1, '!=')->where('is_movie', 3, '!=')->where('is_movie', 2, '!=')->get(T_VIDEOS, $limit,array('video_id','user_id'));
}
