	...
	if ($_POST['type'] == 'get_chats') {
		...
		foreach ($users as $key => $m_user) {
	        ...
	        if (!empty($user)) {
	            ...
	            $get_count_seen = $db->where("to_id = {$pt->user->id} AND from_id = $user->id AND `from_deleted` = '0' AND `to_deleted` = 0 AND seen = 0")->orderBy('id', 'DESC')->getValue(T_MESSAGES, 'COUNT(*)');
...
//			$chat_user->text_time = PT_Time_Elapsed_String($get_last_message->time);
			//
