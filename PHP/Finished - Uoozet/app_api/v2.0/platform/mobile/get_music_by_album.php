...
// +------------------------------------------------------------------------+
if (empty($_GET['album_id']) || !in_array($_GET['album_id'], array_keys($alboums))) {
	$response_data       = array(
        'api_status'     => '400',
	...
	$catid  = PT_Secure($_GET['album_id']);

	if (!empty($offset)) {
		$db->where('id', $offset, '<');
	}

	$videos = $db->where('country',$catid)->where('is_movie', $music_type)->orderBy('id','DESC')->get(T_VIDEOS,$limit,array('video_id','user_id'));
