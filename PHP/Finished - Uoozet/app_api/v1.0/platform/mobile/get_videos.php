...
$db->where('is_movie', 0)->where('featured', '1')->orderBy('RAND()');
$featured = array();
...
if (empty($featured)) {
    ...
    $featured = $db->where('is_movie', 0)->orderBy('id', 'DESC')->get(T_VIDEOS,$limit,array('video_id','user_id'));
}
...
$top   = $db->where('is_movie', 0)->orderby('views', 'DESC')->get(T_VIDEOS, $limit,array('video_id','user_id'));
...
$limit  = ((!empty($get_params['limit'])) ? $get_params['limit'] : 10);
$latest = $db->where('is_movie', 0)->orderby('id', 'DESC')->get(T_VIDEOS, $limit,array('video_id','user_id'));
