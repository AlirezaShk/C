...
function PT_UserData($user_id = 0, $options = array()) {
    ...
    $fetched_data->admin   = $fetched_data->admin;
    ...
    $fetched_data->last_active = $fetched_data->last_active;
    $fetched_data->balance = explode("_", $fetched_data->balance)[0];
...
function PT_GetVideoByID($video_id = '', $add_views = 0, $likes_dislikes = 1, $run_query = 1, $short_id = 0) {
    global $pt, $db, $categories, $ucategories, $sub_ucategories, $singers, $alboums;
    ...
    if (!empty($get_video)) {
        ...
        $get_video->sell_video = ($get_video->price === 0) ? 0 : abs(intval($get_video->price));
        ...
        if (strpos($get_video->thumbnail, 'upload/photos') !== false) {
            ...
            $get_video->thumbnail      = PT_GetMedia2($get_video->thumbnail);

        if ($get_video->type == 4) {
            $get_video->video_location = urldecode($get_video->video_location);
        }
        else{
            $get_video->video_location = PT_GetMedia2($get_video->video_location);
        }
        ...
        if($get_video->is_movie == 2)
            $get_video->url                = PT_Link('listen/' . PT_Slug($get_video->title, $get_video->video_id));
        else
        ...
        $get_video->singer_name           = $get_video->singer? $singers[$get_video->singer]:'';
        $get_video->alboum_name = (!empty($alboums[$get_video->alboum_name])) ? $alboums[$get_video->alboum_name] : '';
        if (IS_LOGGED == true) {
...
//        $get_video->time_alpha    = gmdate('d M Y', $get_video->time);
        $jDate = new jDateTime();
        $get_video->time_alpha    = $jDate->toJalali2($jDate->gDate('Y', $get_video->time), $jDate->gDate('m', $get_video->time), $jDate->gDate('d', $get_video->time));
        $get_video->time_ago      = PT_Time_Elapsed_String($get_video->time);
        $get_video->category_name = (!empty($categories[$get_video->category_id])) ? $categories[$get_video->category_id] : ((!empty($ucategories[$get_video->owner->id][$get_video->category_id])) ? $ucategories[$get_video->owner->id][$get_video->category_id] : '');
        if($get_video->owner->admin && strlen($get_video->category_name) == 0){
            foreach ($ucategories[$get_video->owner->id] as $key => $value){
                if ($sub_ucategories[$key] != '') {
                    for ($i = 0; $i < count($sub_ucategories[$key]); $i++) {
                        foreach ($sub_ucategories[$key][$i] as $key2 => $value2) {
                            if ($get_video->category_id == explode("sub__", $key2)[1]) {
                                $get_video->category_name = $value2;
                                break;
                            }
                        }
                        if (strlen($get_video->category_name) != 0)
                            break;
                    }
                }
                if(strlen($get_video->category_name) != 0)
...
//        $get_video->is_channel == 1?$get_video->vimeo = 1: $get_video->vimeo = 0;
        return $get_video;
    }
    return array();
}
...
function PT_GetMediaByID($video_id = '', $add_views = 0, $likes_dislikes = 1, $run_query = 1, $short_id = 0) {
    global $pt, $db, $categories, $singers, $alboums;
    ...
    if (!empty($get_video)) {
        ...
        $get_video->is_owner           = false;
        $get_video->singer_name           = $get_video->singer? $singers[$get_video->singer]:'';
        $get_video->alboum_name = (!empty($alboums[$get_video->country])) ? $alboums[$get_video->country] : '';
        if (IS_LOGGED == true) {
            $get_video->is_liked    = $db->where('user_id', $pt->user->id)->where('video_id', $get_video->id)->where('type', 1)->getValue(T_DIS_LIKES, 'count(*)');
            $get_video->is_disliked = $db->where('user_id', $pt->user->id)->where('video_id', $get_video->id)->where('type', 2)->getValue(T_DIS_LIKES, 'count(*)');
            if ($get_video->owner->id == $pt->user->id || PT_IsAdmin()) {
...
function PT_GetMusicByID($video_id = '', $add_views = 0, $likes_dislikes = 0, $run_query = 1, $short_id = 0) {
    global $pt, $db, $musicCategories, $singers, $alboums;
    ...
    if (!empty($get_video)) {
        ...
        $get_video->org_thumbnail = $get_video->thumbnail;
        $get_video->music_id     = $get_video->video_id;
        if (strpos($get_video->thumbnail, 'upload/photos') !== false) {
            $get_video->thumbnail      = PT_GetMedia($get_video->thumbnail);
            $get_video->source         = 'Uploaded';
            $get_video->music_type     = 'audio/mp3';

            if ($get_video->type == 4) {
                $get_video->music_location = urldecode($get_video->video_location);
            }

            else{
                $get_video->music_location = PT_GetMedia($get_video->video_location);
            }
...
//        $get_video->time_alpha    = gmdate('d M Y', $get_video->time);
        ...
        $get_video->time_alpha    = $jDate->toJalali2($jDate->gDate('Y', $get_video->time), $jDate->gDate('m', $get_video->time), $jDate->gDate('d', $get_video->time));
        $get_video->time_ago      = PT_Time_Elapsed_String($get_video->time);
        $get_video->category_name = (!empty($musicCategories[$get_video->category_id])) ? $musicCategories[$get_video->category_id] : '';
        $get_video->singer_name = (!empty($singers[$get_video->singer])) ? $singers[$get_video->singer] : '';
        $get_video->alboum_name = (!empty($alboums[$get_video->country])) ? $alboums[$get_video->country] : '';
        if ($likes_dislikes == 1) {
            $db->where('video_id', $get_video->id);
            $db->where('type', 1);
        ...
        return removeForMusics($get_video);
    }
    return array();
}

function removeForMusics($get_video){
//    unset($get_video->video_id);
    unset($get_video->video_location);
    unset($get_video->youtube);
    unset($get_video->vimeo);
    unset($get_video->daily);
    unset($get_video->facebook);
    unset($get_video->twitch);
    unset($get_video->twitch_type);
    unset($get_video->sell_viceo);
//    unset($get_video->is_movie);
    $get_video->music_release = $get_video->movie_release;
    $get_video->alboum_id = $get_video->country?$get_video->country:'';
    unset($get_video->movie_release);
    unset($get_video->producer);
    unset($get_video->rent_price);
    unset($get_video->stars);
    unset($get_video->sell_video);
    unset($get_video->sub_category);
    unset($get_video->geo_blocking);
    unset($get_video->quality);
    unset($get_video->demo);
    unset($get_video->{'240p'});
    unset($get_video->{'360p'});
    unset($get_video->{'480p'});
    unset($get_video->{'720p'});
    unset($get_video->{'1080p'});
    unset($get_video->{'2048p'});
    unset($get_video->{'4096p'});
    return $get_video;
}

function removeForAPI($getVideo){
    unset($getVideo->video_id);
    unset($getVideo->country);
    return $getVideo;
}
...
function PT_GetMedia2($media = '', $is_upload = false){
    global $pt;
    if (empty($media)) {
        return '';
    }
...
function PT_MobileExists($username = '') {
    global $db;
    return ($db->where('mobile', PT_Secure($username))->getValue(T_USERS, 'count(*)') > 0) ? true : false;
}

function PT_UsernameDetail($username = '') {
    global $db;
    $db->where('username', $username)->orWhere('mobile', $username);
    $user = $db->getOne(T_USERS);
    return(empty($user) ? 0 : $user);
}

function PT_MobileDetail($mobile = '') {
    global $db;
...
function PT_IsCodeValid($username, $code){
    global $db;
...
function sendSMS($to, $code, $line = 5000106000497){
    ini_set("soap.wsdl_cache_enabled", "0");
    $sms_client = new SoapClient('http://api.payamak-panel.com/post/send.asmx?wsdl', array('encoding'=>'UTF-8'));
//
//    $parameters['username'] = "09121036706";
//    $parameters['password'] = "1411";
//    $parameters['to'] = substr($to, 1);
...
function PT_ShareFile($data = array(), $type = 0) {
    ...
    if (!file_exists('upload/subtitle/' . date('Y'))) {
        @mkdir('upload/subtitle/' . date('Y'), 0777, true);
    }
    if (!file_exists('upload/subtitle/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/subtitle/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists('upload/musics/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/musics/' . date('Y') . '/' . date('m'), 0777, true);
    }
    ...
    $allowed           = 'jpg,png,jpeg,gif,mp4,mov,webm,mpeg,3gp,mkv,mk3d,mks,mp3,srt,vtt';
    ...
    if (!in_array($file_extension, $extension_allowed)) {
        return array(
            'error' => 'File format not supported1'
        );
    }
    if ($file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif') {
        $folder   = 'photos';
        $fileType = 'image';
    } elseif ($file_extension == 'srt' || $file_extension == 'vtt') {
        $folder   = 'subtitle';
        $fileType = 'text';
    ...
    if (!in_array($data['type'], $ar)) {
        return array(
            'error' => 'File format not supported2',
            'type' => $data['type']
        );
    }
...
function PT_DeleteVideo($id = 0) {
    ...
    $delete = $db->where('media_id', $id)->delete(T_CAT_VIDEOS);
...
function PT_GetLastSeen($id, $data = array(),$limit = 50) {
    global $pt, $db;
    ...
    foreach ($get_user_messages as $key => $message) {
        if ($return_method == 'html') {
            ...
            if(substr($message->text, 0, 7) != 'shared_'){
                ...
                $typeMsg = 'media';
                $txt = '';
                if(substr($message->text, 7) == 'media'){
                    $video = PT_GetVideoByID($message->media,0,0,2);
                    if($video->is_movie > 1)
                        $video = PT_GetMusicByID($message->media,0,0,2);
                    $txt = $video->url;
                    $title = $video->title;
                    $pic = $video->thumbnail;

                }else{
                    $video   = $db->where('id', $message->media)->get(T_POSTS);
                    foreach ($video as $key => $post) {
                        ...
                        $u_like     = $db->where('post_id', $post->id)->where('user_id', $pt->user->id)->where('type', 1)->getValue(T_DIS_LIKES, "count(*)");
                        $post->liked      = ($u_like > 0) ? 1 : 0;

                        $u_dislike  = $db->where('post_id', $post->id)->where('user_id', $pt->user->id)->where('type', 2)->getValue(T_DIS_LIKES, "count(*)");
                ...
                if($typeMsg == 'text'){
                    ...
                    $messages_html .= PT_LoadPage("messages/ajax/$message_type-media", array(
                        'ID' => $message->id,
                        'AVATAR' => $chat_user->avatar,
                        'NAME' => $chat_user->name,
                        'TEXT' => $txt,
