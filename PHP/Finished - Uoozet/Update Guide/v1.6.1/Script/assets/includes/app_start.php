...
$musics_categories = array();
try {
    $all_musics_categories = $db->where('type','music_category')->get(T_LANGS);
    if (!empty($all_music_categories)) {
        ...
        foreach ($all_music_categories as $key => $value) {
            ...
            if ($value->lang_key != 'other') {
                if (!empty($value->lang_key) && !empty($lang->{$value->lang_key})) {
                    $musics_categories[$value->lang_key] = $lang->{$value->lang_key};
                }
            }
            if (end($array_keys) == $key) {
                $musics_categories['other'] = $lang->other;
            }
        }
    }
    else{
...
$pt->musics_categories = $musics_categories;
