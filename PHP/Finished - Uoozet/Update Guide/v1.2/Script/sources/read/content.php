...
$get_comments   = $db->where('post_id',$post_id)->orderBy('id','desc')->get(T_COMMENTS,20);
...
foreach ($get_comments as $get_comment) {
	...
	//Get comment relatet likes
    $db->where('comment_id', $get_comment->id);
    $db->where('type', 1);
    $comment_likes        = $db->getValue(T_COMMENTS_LIKES, 'count(*)');

    //Get comment relatet dislikes
    $db->where('comment_id', $get_comment->id);
    $db->where('type', 2);
