<?php
$comments = '';
if (!empty($_POST['last_id']) && !empty($_POST['video_id'])) {
    $last_id  = PT_Secure($_POST['last_id']);
    $video_id = PT_Secure($_POST['video_id']);
    $get_comments = $db->where('id', $last_id, '<')->where('video_id', $video_id)->orderBy('id', 'DESC')->get(T_COMMENTS, 10);
    if (count($get_comments) > 0) {
        foreach ($get_comments as $key => $comment) {
	        ...
	        //Get comment related likes 
	        $db->where('comment_id', $comment->id);
	        $db->where('type',1);
	        $comments_likes   = $db->getValue(T_COMMENTS_LIKES, 'count(*)');

	        //Get comment related likes
	        $db->where('comment_id', $comment->id);
	        $db->where('type',2);
	        $comment_dislikes = $db->getValue(T_COMMENTS_LIKES, 'count(*)');
