...
if (!empty($_GET['first']) && !empty($_POST['id'])) {
    ...
    $is_this_valid_comment = $db->where('id', $id)->getValue(T_COMMENTS, 'count(*)');
    if ($is_this_valid_comment > 0) {
        ...
        $data['up']    = $db->where('comment_id', $id)->where('type', 1)->getValue(T_COMMENTS_LIKES, "count(*)");
        $data ['down'] = $db->where('comment_id', $id)->where('type', 2)->getValue(T_COMMENTS_LIKES, "count(*)");
    }
}
?>