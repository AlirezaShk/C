...
{
    if($_SESSION['temp_httpRef'] == PT_Link("loginWithPhone"))
    {
        $_SESSION['temp_httpRef'] = "verify";
...
if(strpos($page,"movie-sharing") !== FALSE && !PT_IsAdmin()){
    $page = "home";
}
if(strpos($page,"audio_book") !== FALSE && !PT_IsAdmin()){
    $page = "home";
}
if (IS_LOGGED == true) {
...
$seo_info = $sd->getOne($pageURL);
$page_meta_desc = $seo_info['desc'];
$page_canonical = "";
foreach (json_decode($seo_info['canonical']) as $canonical) {
    $page_canonical .= "<link rel='canonical' href='".$canonical."'/>";
}
/*       END      */
if($pt->theme_using == 'mana' && $page != 'import-video-api'){
//    echo PT_LoadPage("header/$side_header");exit;

    $final_content = PT_LoadPage('container-new', array(
        'CONTAINER_TITLE' => $pt->title,
        /*'CONTAINER_DESC' => $pt->description,*/
        'CONTAINER_DESC' => $page_meta_desc,
        'CONTAINER_CANONICAL' => $page_canonical,
