...
$related_videos = $articleManager->getRelatedVideos($post_id, 1, 0, 0, 0, 1, TRUE, FALSE, Video::RETURN_OBJECT);
for ($i_ = 2; $i_ >= 0; $i_--) {
    foreach ($related_videos->{$i_} as $key => $related_video) {
        $related_video  = PT_GetVideoByID($related_video, 0, 0, 2);
        $videos_list .= PT_LoadPage('watch/video-sidebar', array(
            'ID' => $related_video->id,
