...
if ($page == 'trending') {
    ...
    // pagination system
    $videos = $db->where('privacy', 0)->where('time', time() - (TRENDING_VIDEO_DAY_PERIOD * 30 * 24 * 3600), '>')->where('is_movie',0)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $videos = sortVideosByTrendScore($videos);
    ...
    // pagination system
    $videos = $db->where('privacy', 0)->where('is_movie',0)->where('is_channel',0)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system
}
    ...
    // pagination system
    $videos = $db->where('privacy', 0)->where('is_movie',0)->where('is_channel',0)->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system
}
    ...
    if (!empty($_GET['id'])) {
        ...
        if ($getCats->catHasVid($_GET['id'])) {
            $pt->page_url_ = $pt->config->site_url.'/videos/'.$page.'/'.$_GET['id'].'?page_id='.$pt->page_number;
            $cateogry = $_GET['id'];
            $catTitle = $langManager->getOneById($cateogry);
            $title    = $catTitle[$_SESSION['lang']];
...
//            $videos = $db->rawQuery("SELECT * FROM `".T_VIDEOS."` WHERE `privacy`=0 AND `category_id`='".$cateogry."' AND `is_movie`!=1 ORDER BY `id` DESC");
//            $videos = $db->rawQuery("SELECT * FROM `".T_VIDEOS."` WHERE `category_id`=605 ORDER BY `id` DESC LIMIT 20");
//            $videos = $db->rawQuery("SELECT * FROM `".T_VIDEOS."` WHERE `id`=95 ORDER BY `id` DESC LIMIT 20");
//            $videos = $db->rawQuery("SELECT * FROM `".T_LANGS."` WHERE `id`=660 ORDER BY `id` DESC LIMIT 20");
//            print_r($videos);
...
//            exit();
...
} else if($page == 'likedCats') {

    $title  = $lang->likedCats;
    $likedCats_list = '';
    ...
    if($likedCats) {
        ...
        $videos = $db->where('is_movie', 1, '!=')->where('is_movie', 3, '!=')->where('is_movie', 2, '!=')->where('category_id', $ids_array, 'IN')->orderBy('id', 'DESC')->get(T_VIDEOS);
    }
}
...
$pt->videos_count= count( (array) $videos);
$pt->page        = $page;
