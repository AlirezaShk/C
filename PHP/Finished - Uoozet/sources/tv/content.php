...
if ($type == 'views') {
	if ($sort_type == 'all_time') {
		$videos = $db->rawQuery('SELECT user_id, SUM(views) AS count FROM '.T_VIDEOS.' join users on users.id = videos.user_id where users.admin = 1 and users.username != "admin" and users.username != "AlirezaShk" GROUP BY user_id ORDER BY count DESC LIMIT 20');
	}
	else{
...
if (!empty($videos)) {
	foreach ($videos as $key => $value) {
    	...
    	if (!empty($user)) {
    		...
    		$final2 .= PT_LoadPage('tv/list', array(
			    'ID' => $user->id,
			    'USER_DATA' => $user,
			    'VIEWS' => $views_count,
			    'VIEWS_COUNT' => $views_,
...
$pt->page_url_ = $pt->config->site_url.'/tv?type='.$type.'&sort_type='.$sort_type;
$pt->page        = 'tv';
$pt->title       = $lang->popular_channels . ' | ' . $pt->config->title;
$pt->description = $pt->config->description;
$pt->keyword     = $pt->config->keyword;
$pt->content     = PT_LoadPage('tv/content', array(
    'CHANNELS' => $final,
    'TV_PADDING' => intval(($cnt-1)/3) * 5
));