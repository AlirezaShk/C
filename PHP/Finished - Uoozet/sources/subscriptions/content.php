...
$final = '';
$channels = '<div class="items">';
$channels2 = '<div class="items">';

$get = $db->where('subscriber_id', $user->id)->get(T_SUBSCRIPTIONS);
$get2 = $db->where('user_id', $user->id)->get(T_SUBSCRIPTIONS);
$userids = array();
...
//    $userdata = array_intersect_key(ToArray($userdata), array_flip($user_public_data));
    if (!empty($userdata)) {
        $channels .= '<div class="followItem item"><a href="'.$userdata->url.'"><img src="'.$userdata->avatar.'"><span>'.$userdata->username.'</span></a></div>';
    }
}
...
//    $channels2 = $userdata2;
    if (!empty($userdata2)) {
        $channels2 .= '<div class="followItem item"><a href="'.$userdata2->url.'"><img src="'.$userdata2->avatar.'"><span>'.$userdata2->username.'</span></a></div>';
    }
}
