...
$videoManager->setBinary_conditions(['id'=>$get_video->id]);
$vid = $videoManager->getMatches();
...
if ($get_video->sell_video > 0) {
    if (!empty($user->id)) {
        $paidList = (array) json_decode($vid['paid_for']);
        $pt->is_paid = ( (key_exists($user->id, $paidList) === false) ? 0 : 1 );
    }
    $pt->purchased = count( (array) json_decode( $vid['paid_for']));
}
...
//$related_videos = $videoManager->getRelatedVideos($get_video->id, 1, 0, 0, 0, 1, TRUE, FALSE, Video::RETURN_OBJECT);
$related_videos = $videoManager->getRelatedVideos($get_video->id, 1, 1, 1, 1, 1, TRUE, FALSE, Video::RETURN_OBJECT);
...
for ($i_ = 2; $i_ >= 0; $i_--) {
    foreach ($related_videos->{$i_} as $key => $related_video) {
...
if ($last_ads == 0 && !$is_pro && $ads_sys) {
    $rand      = 1;//(rand(0,1)) ? rand(0,1) :(rand(0,1) ? : rand(0,1));

    if ($rand == 0) {
        $get_random_ad = $db->where('active', 1)->where('type', 'begin')->orderBy('RAND()')->getOne(T_VIDEO_ADS);
        $get_end_ad = $db->where('active', 1)->where('type', 'end')->orderBy('RAND()')->getOne(T_VIDEO_ADS);
        ...
        if (!empty($get_end_ad)) {
            ...
            if (!empty($get_end_ad->ad_media)) {
                $end_ad_media = $get_end_ad->ad_media;
                $end_ad_link = PT_Link('redirect/' . $get_end_ad->id . '?type=video');
                $end_is_video_ad = ",'ads'";

            }

            if (!empty($get_end_ad->vast_xml_link)) {
                $end_vast_url = $get_end_ad->vast_xml_link;
                $end_vast_type = $get_end_ad->vast_type;
                $end_is_vast_ad = ",'vast'";
            }

            if ($get_end_ad->skip_seconds > 0) {
                $end_ad_skip = 'true';
                $end_ad_skip_num = $get_end_ad->skip_seconds;
            }

            if (!empty($get_end_ad->ad_image)) {
                $end_ad_image = $pt->ad_image = $get_end_ad->ad_image;
                $end_ad_link = PT_Link('redirect/' . $get_end_ad->id . '?type=image');
            }

            $update_clicks = $db->where('id', $get_end_ad->id)->update(T_VIDEO_ADS, array(
                'views' => $db->inc(1)
            ));
        }
    }
...
//    if ($pt->get_video->sell_video == 0 || PT_IsAdmin() || $pt->get_video->is_owner || ($pt->get_video->sell_video > 0 && $pt->is_paid > 0 && $pt->config->sell_videos_system == 'on')) {
    if (intval($vid['price']) == 0 || PT_IsAdmin() || $pt->get_video->is_owner || (intval($vid['price']) > 0 && $pt->is_paid > 0 && $pt->config->sell_videos_system == 'on')) {
        if ($get_video->{"240p"} == 1) {
    ...
    else if ((intval($vid['price']) !== 0) && !$pt->is_paid && $pt->config->sell_videos_system == 'on' && !empty($get_video->demo)) {
        $quality_set = false;
        if ($get_video->{"4096p"} == 1) {
            $pt->video_quality = '4K';
            $pt->video_res = '4096';
...
foreach (PT_GetAllUsers() as $u){
    if(PT_GetUserFromSessionID($_SESSION['user_id']) != $u->id)
        $options .= '<option value="'.$u->id.'">'.$u->name.'('.$u->username.')</option>';
}
//print_r($get_video->video_location);

if ($get_video->owner->admin == 1 && ($get_video->owner->id !== 1 && $get_video->owner->username !== "AlirezaShk")) {
    header("Location: " . PT_Link('') . "@" . $get_video->owner->username . "?vid=" . $get_video->id);
    exit();
}

$catHTML = "";
$cats = explode("|", $get_video->categories);
$lm = new Lang();
foreach ($cats as $cat) {
    if (strlen($cat) === 0) continue;
    $cat = $lm->getOneById($cat);
    $catHTML .= '<div class="card" data-id="'.$cat['id'].'"><a class="home" href="/videos/category/'.$cat['id'].'">'.$cat[$_SESSION['lang']].'</a></div>';
}
$subtitle = $get_video->subtitle;
$pt->content = PT_LoadPage("watch/$content_page", array(
    ...
    'URL' => $get_video->url,
    'SUB' => PT_Link($get_video->subtitle),
    'VIDEO_CATS' => $catHTML,
    ...
    'USERS_LIST'=>$options,
    ////
    'END_AD_MEDIA' => "'$end_ad_media'",
    'END_AD_LINK' => "'$end_ad_link'",
    'END_AD_P_LINK' => "$end_ad_link",
    'END_AD_SKIP' => $end_ad_skip,
    'END_AD_SKIP_NUM' => $end_ad_skip_num,
    'END_ADS' => $end_is_video_ad,
    'END_USER_ADS_DESC_OVERLAY' => $ad_desc,
    'END_VAT' => $end_is_vast_ad,
    'END_AD_IMAGE' => $end_ad_image,
    ///
