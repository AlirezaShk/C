...
$pt->page_url_ = $pt->config->site_url.'/listen/'.PT_Slug($get_video->title, $get_video->video_id);
...
if (!empty($_GET['list']) && $_GET['list'] == 'wl' && IS_LOGGED) {
    ...
    if (!$pt->is_wl) {
        header("Location: " . PT_Link("listen/$id"));
        exit();
    }
    $pt->page_url_ = $pt->config->site_url.'/listen/'.PT_Slug($get_video->title, $get_video->video_id).'/list/'.$_GET['list'];
...
if ($pt->is_wl === true && IS_LOGGED === true) {
    ...
    foreach ($wl_list_videos as $key => $pl_vid) {
        ...
        $pl_vid->url    = PT_Link('listen/' . PT_Slug($pl_vid->title, $pl_vid->video_id) . "?list=wl");
        $list_sidebar .= PT_LoadPage('listen/wl-list', array(
            'TITLE' => $pl_vid->title,
            'URL' => $pl_vid->url,
            'VID_ID' => $pl_vid->id,
            'ID' => $pl_vid->video_id,
    ...
    foreach ($play_list_videos as $key => $pl_vid) {
        $pl_vid         = PT_GetVideoByID($pl_vid, 0, 0, 0);
        $pl_vid->url    = PT_Link('listen/' . PT_Slug($pl_vid->title, $pl_vid->video_id) . "/list/$list_id");
        $list_sidebar .= PT_LoadPage('listen/video-list', array(
            'TITLE' => $pl_vid->title,
            'URL' => $pl_vid->url,
            'LIST_ID' => $list_id,
            'VID_ID' => $pl_vid->id,
...
foreach ($related_videos as $key => $related_video) {
    ...
    $video_sidebar .= PT_LoadPage('listen/video-sidebar', array(
        'ID' => $related_video->id,
...
if (!empty($get_video_comments)) {
    ...
    foreach ($get_video_comments as $key => $comment) {
        ...
        foreach ($comment_replies as $reply) {
            ...
            $replies    .= PT_LoadPage('listen/replies', array(
                'ID' => $reply->id,
        ...
        $comments     .= PT_LoadPage('listen/comments', array(
            'ID' => $comment->id,
...
$ad_image = '';
$ad_link = '';
$sidebar_ad = PT_GetAd('listen_side_bar');
$is_pro  = false;
$user_ad_trans = '';
$ad_desc = '';
...
// demo video
$content_page = (($pt->is_list === true) ? "playlist" : (($pt->is_wl === true) ? "listen-later" : "content"));
...
$pt->content = PT_LoadPage("listen/$content_page", array(
    'ID' => $get_video->id,
    ...
    'VIDEO_COMMENTS' => PT_LoadPage('listen/video-comments',array(
