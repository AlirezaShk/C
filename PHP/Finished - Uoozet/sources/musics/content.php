...
$videos = array();
if ($page == 'trending') {
    $title  = $lang->trending_music;
    // $db->where('privacy', 0);
    // $videos = $db->where('time', time() - 172800, '>')->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);

    // pagination system 
    $videos = $db->where('privacy', 0)->where('time', time() - 1036800, '>')->where('is_movie',2)->where('is_movie',1, '>')->where('is_movie', 4, '<')->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('is_movie',1, '>')->where('is_movie', 4, '<')->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 

else if ($page == 'top') {
    $title  = 'برترین پادکست ها';
    // $db->where('privacy', 0);
    // $videos = $db->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);

    // pagination system 
    $videos = $db->where('privacy', 0)->where('is_movie',1, '>')->where('is_movie', 4, '<')->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
    ...
    if (!empty($_GET['id'])) {
        if (in_array($_GET['id'], array_keys($musicCategories)) || in_array($_GET['id'], array_keys($radioCategories))) {
            $pt->page_url_ = $pt->config->site_url.'/musics/'.$page.'/'.$_GET['id'].'?page_id='.$pt->page_number;
            $cateogry = PT_Secure($_GET['id']);
            if(in_array($_GET['id'], array_keys($musicCategories)))
                $title    = $musicCategories[$cateogry];
            else
                $title    = $radioCategories[$cateogry];
            ...
            if (!empty($_GET['sub_id'])) {
                ...
                if ($is_found > 0) {
                    $pt->page_url_ = $pt->config->site_url.'/musics/'.$page.'/'.$_GET['id'].'/'.$_GET['sub_id'].'?page_id='.$pt->page_number;
            ...
            // pagination system 
            $videos = $db->where('privacy', 0)->where('category_id = "' . $cateogry.'"')->where('is_movie',1, '>')->where('is_movie', 4, '<')->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
            $pt->total_pages = $db->totalPages;
            // pagination system 
    ...
    if (!empty($_GET['id'])) {
        if (in_array($_GET['id'], array_keys($singers))) {
            $pt->page_url_ = $pt->config->site_url.'/musics/'.$page.'/'.$_GET['id'].'?page_id='.$pt->page_number;
            $singer = PT_Secure($_GET['id']);
            $title    = $singers[$singer];
            $singer_id = "data-category='$singer'";


            // pagination system
            $videos = $db->where('privacy', 0)->where('singer = "' . $singer.'"')->where('is_movie',1, '>')->where('is_movie', 4, '<')->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
            $pt->total_pages = $db->totalPages;
            // pagination system
    ...
    if (!empty($_GET['id'])) {
        if (in_array($_GET['id'], array_keys($alboums))) {
            $pt->page_url_ = $pt->config->site_url.'/musics/'.$page.'/'.$_GET['id'].'?page_id='.$pt->page_number;
            $alboum = PT_Secure($_GET['id']);
            $title    = $alboums[$alboum];
            $alboum_id = "data-alboum='$alboum_id'";


            // pagination system
            $videos = $db->where('privacy', 0)->where('country = "' . $alboum.'"')->where('is_movie',1, '>')->where('is_movie', 4, '<')->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
            $pt->total_pages = $db->totalPages;
            // pagination system
