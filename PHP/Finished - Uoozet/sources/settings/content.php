...
$lm = new Lang();
$time_periods = $lm->getTimePeriods();
$time_period_options = "";
foreach ($time_periods as $tp) {
    if($tp['lang_key'] == 'monthly')
        $time_period_options .= "<option value='".$tp['lang_key']."'>".$tp[$_SESSION['lang']]."</option>";
}
$currencies = $lm->getCurrencies();
$currency_options = "";
foreach ($currencies as $cur) {
    $currency_options .= "<option value='".$cur['lang_key']."'>".$cur[$_SESSION['lang']]."</option>";
}
$channels_html = "";
$chs = new Channels();
$chs->getAll();
...
$pt->content     = PT_LoadPage("settings/content", array(
    'SETTINGSPAGE' => PT_LoadPage("settings/$pt->setting_page", array(
        ...
        'CHARGE_PERIOD_OPTIONS' => $time_period_options,
        'CURRENCY_OPTIONS' => $currency_options,
        'ADMIN_LAYOUT' => PT_LoadPage('settings/admin', array(
            'USER_DATA' => $pt->settings
        )),
        'CHANNEL_LIST' => $channels_html
    ))
));
