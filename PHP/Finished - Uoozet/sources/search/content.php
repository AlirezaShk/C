...
$list3 = '<div class="text-center no-content-found empty_state"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-video-off"><path d="M16 16v1a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h2m5.66 0H14a2 2 0 0 1 2 2v3.34l1 1L23 7v10"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>موزیکی برای عبارت مورد جستجو وجود ندارد</div>';
...
// Ready Keyword for crawling the DB:
function returnSearchText_Array(string $keyword) {
    $regex = "/[^A-z0-9\sابپتثجچحخدذرزژسشصضطظعغفقکگلمنوهی]+/";
    $search_txt = preg_replace($regex, " ", $keyword);
    $regex = "/[\s]{2,}/";
    $search_txt = preg_replace($regex, " ", $search_txt);
    $searchArray = explode(" ", $search_txt);
    $validArray = array();
    for ($i = 0; $i < count($searchArray); $i++) {
        if (strlen($searchArray[$i]) > 0) {
            $validArray[] = $searchArray[$i];
        }
    }
    return $validArray;
}
function returnKeywordCond($col, $searchPatterns) {
    $keywordCond = "(";
    for ($i = 0; $i < count($searchPatterns); $i++) {
        $keywordCond .= "`$col` LIKE '%". $searchPatterns[$i] ."%'";
        if ($i != (count($searchPatterns) - 1)) {
            $keywordCond .= " OR ";
        }
    }
    $keywordCond .= ")";
...
$searchPatterns = returnSearchText_Array($keyword);
...
//    $get_videos = $db->where("MATCH (title) AGAINST ('$keyword') AND privacy = 0  ".$category.$date)->orderBy('id', 'ASC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $get_videos = $db->where(returnKeywordCond("title", $searchPatterns) . " AND privacy = 0  ".$category.$date)->orderBy('id', 'ASC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    ...
    foreach ($pt->singers as $key=>$item){
        if(strpos($item, $keyword))
            $singer = $key;
    }
    // pagination system
//    $get_videos = $db->where("(title LIKE '%$keyword%' OR tags LIKE '%$keyword%' OR description LIKE '%$keyword%' OR singer = '$singer') AND privacy = 0  ".$category.$date)->orderBy('id', 'ASC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
//    echo "(".returnKeywordCond("title", $searchPatterns)." OR ".returnKeywordCond("tags", $searchPatterns)." OR ".returnKeywordCond("description", $searchPatterns)." OR singer = '$singer') AND privacy = 0  ".$category.$date;
    $get_videos = $db->where("(".returnKeywordCond("title", $searchPatterns)." OR ".returnKeywordCond("tags", $searchPatterns)." OR ".returnKeywordCond("description", $searchPatterns)." OR singer = '$singer') AND privacy = 0  ".$category.$date)->orderBy('id', 'ASC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system
}
if (!empty($get_videos)) {
    ...
    foreach ($get_videos as $key => $video) {
        ...
        if($video->is_movie == 2 || $video->is_movie == 3){
            ...
            $final3 .= PT_LoadPage('search/list', array(
                'ID' => $video->id,
...
if (!empty($get_articles)) {
    $len = count($get_articles);
    foreach ($get_articles as $key => $article) {
        $pt->last_video = false;
        if ($key == $len - 1) {
            $pt->last_video = true;
        }
        $final4 .= PT_LoadPage('search/list', array(
            'ID' => $article->id,
            'USER_DATA' => null,
