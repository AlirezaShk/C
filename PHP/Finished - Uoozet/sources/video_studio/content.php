...
//if(isset($_GET['user_id'])) $pt->user = $user = PT_UserData(intval($_GET['user_id']));
...
if (!empty($_GET['videos_type']) && $_GET['videos_type'] == 'movies') {
	$pt->page_url_ = $pt->config->site_url.'/video_studio/movies' . ((isset($_GET['user_id'])) ? ("/".$_GET['user_id']) : (""));
}
else{
	$pt->page_url_ = $pt->config->site_url.'/video_studio' . ((isset($_GET['user_id'])) ? ("/".$_GET['user_id']) : (""));
}
...
$pt->content = PT_LoadPage('video_studio/content', array(
