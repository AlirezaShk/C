...
}elseif (PT_IsAdmin() && !empty($_POST['is_movie']) && $_POST['is_movie'] == 2){
...
if(empty($_POST['isChannelVid']))
	$isChannelVid = 0;
else
	$isChannelVid = $_POST['isChannelVid'];
if (empty($error)) {
    $id = PT_Secure($_POST['video-id']);
...
//		            $category_id = array_search(PT_Secure($_POST['category_id']),get_object_vars($pt->categories));
//		        }
//		        else{
//					foreach ($pt->ucategories as $k => $ucat){
//						if (in_array($_POST['category_id'], get_object_vars($ucat))) {
//							$category_id = array_search($_POST['category_id'], get_object_vars($ucat));
//							break;
//						}
//						$key_ = explode(" -- ", $_POST['category_id'])[0];
//						$val_ = explode(" -- ", $_POST['category_id'])[1];
//						$category_id = $key_;
////						foreach ($ucat as $k_ => $ucat_) {
////							foreach ($pt->sub_ucategories[$k_] as $k2 => $sub_ucat) {
////								if (intval($key_) == $k2) {
//////								if (in_array($post_, $sub_ucat)) {
////									$category_id = explode("sub__",array_search($val_, $sub_ucat))[1];
////									break;
////								}
////							}
////						}
