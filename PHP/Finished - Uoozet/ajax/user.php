...
if ($first == 'general') {
    if (empty($_POST['username']) OR empty($_POST['email'])) {
        ...
        if (!empty($user_data->id)) {
            ...
            $allow_notify = $user_data->is_pro;
            if (isset($_POST['allow_notify']) && PT_IsAdmin()) {
                if ($_POST['allow_notify'] == 1) {
                    $allow_notify = 1;
                }

                else if ($_POST['allow_notify'] == 0) {
                    $allow_notify = 0;
                }
            }
    ...
    if (!empty($user_data->id)) {
        ...
        if (!empty($_FILES['cover']['tmp_name'])) {
            $file_info = array(
                ...
                'crop' => array('width' => 1200, 'height' => 400)
            );
            $file_upload = PT_ShareFile($file_info);
            if (!empty($file_upload['filename'])) {
...
//    }
//}
if ($first == 'modify-balance' && !empty($_POST['trans_type']) && !empty($_POST['amount']) && is_numeric($_POST['amount'])) {
    $data['status'] = 400;
    $data['message'] = 'عملیات های مالی هنوز در حال راه اندازی هستند.';
    if(intval($_POST['trans_type']) == 1) {
        /* DEPOSIT */
    } else {
        /* WITHDRAWAL */
    }
