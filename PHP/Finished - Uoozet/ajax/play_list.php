...
$action = $_GET['action'];
$cat_array = explode("|", $_POST['cats']);
$vid_array = explode("|", $_POST['vids']);
$lid = intval($_GET['lid']);
$uid = intval($user->id);
$playlist_manager = new Playlist();
$current = $playlist_manager->getOne($lid);
switch ($action) {
    case "add_vid":
        $playlist_manager->addVidTo($lid, $vid_array);
        break;
    case "del_vid":
        $playlist_manager->delVidFrom($lid, $vid_array);
        break;
    case "add_cat":
        $playlist_manager->addCat($lid, $cat_array);
        break;
    case "del_cat":
        $playlist_manager->delCat($lid, $cat_array);
        break;
    case "add_list":
        $name = $_POST['name'];
        $desc = $_POST['desc'];
        $type = $_POST['type'];
        $playlist_manager->createOne($name, )
        break;
}
