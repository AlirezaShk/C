...
if ($first == 'save-settings') {
...
} else if ($first == 'delete-multi-videos') {
    if (!empty($_POST['ids'])) {
...
} else if ($first == 'load-more-youtube')  {
...
} else if ($first == 'import-daily-videos') {
...
} else if ($first == 'import-youtube-videos') {
...
} else if ($first == 'submit-sitemap-settings') {
    ...
    if (empty($error)) {
        ...
        $insert         = $db->where('id',$id)->update(T_SINGERS,$update_data);
        $data['status'] = ($insert && empty($error)) ? 200 : 500;
        $data['url']    = PT_Link('singers/read/' . PT_URLSlug($update_data['name'],$id));
    }

    else{
        $data['status'] = $error;
    }
} else if ($first == 'create-singer') {
    $error = false;
    ...
    if (empty($error)) {
        ...
        $insert         = $db->insert(T_SINGERS,$update_data);
        $data['status'] = ($insert && empty($error)) ? 200 : 500;
        $data['url']    = PT_Link('singers/read/' . PT_URLSlug($update_data['name'],$id));
    }
    ...
    if (empty($error)) {
        ...
        $users = $db->where('admin',0)->get(T_USERS);
        foreach ($users as $user){
            ...
            $id = $user->id;
            if ($id != $pt->user->id) {
                $chat_exits = $db->where("user_two", $pt->user->id)->where("user_one", $id)->getValue(T_CHATS, 'count(*)');
                if (!empty($chat_exits)) {
...
//        $data['url']    = PT_Link('singers/read/' . PT_URLSlug($update_data['name'],$id));
        ...
        // insert fcm
        define('API_ACCESS_KEY','AAAA3K-Xt74:APA91bEF4McVKg-dFOorv9JSBre-3WMZ3-mxMz5fLkXHJREmWXFe6_B1b0InzF6XF9pXd3Yom3Icpe4PF3JtVRQLCvEinA1idoxpHQl6Jvk_Ah3Vo1YZ_itYytZKdoWDcXvtBTpkEWY5');
        $fcmUrl = 'https://fcm.googleapis.com/fcm/send';
        $token='235zgagasd634sdgds46436';

        $notification = [
            'title' =>substr($_POST['message'],0,50).'...',
            'body' =>$_POST['message'],
            'icon' =>'https://www.uoozet.com/themes/mana/img/LogoForApp.jpg',
            'sound' => 1,
            'type'=>'chat',
            'senderId'=> $pt->user->id,
            'name'=> $pt->user->name,
            'time'=>$pt->user->last_active
        ];
...
} else if ($first == 'delfield' && !empty($_GET['id'])) {
...
} else if ($first == 'withdrawal-requests' && !empty($_POST['id']) && !empty($_POST['a'])) {
...
} else if ($first == 'delete-singer') {
    $request        = (!empty($_POST['id']) && is_numeric($_POST['id']));
    $data['status'] = 400;
    if ($request === true) {
        $singer_id = PT_Secure($_POST['id']);
        $db->where('id',$singer_id)->delete(T_SINGERS);
        $data['status'] = 200;
    }
} else if ($first == 'delete-announcement') {
    $request        = (!empty($_POST['id']) && is_numeric($_POST['id']));
    $data['status'] = 400;
    if ($request === true) {
...
} else if ($first == 'toggle-announcement') {
    $request        = (!empty($_POST['id']) && is_numeric($_POST['id']));
    $data['status'] = 400;
...
} else if ($first == 'banip' && !empty($_POST['ip'])) {
...
} else if ($first == 'save-custom-design-settings') {
    $data     = array('status' => 200);
    $code     = array();
...
if ($first == 'load-more-twitch')  {
   ...
   //                              $tag_count++;
...
} else if ($first == 'import-twitch-videos') {
    if (!empty($_POST['videos'])) {
        $ids = array();
        $category_id = 0;
        if (!empty($_POST['category_id'])) {
...
} else if ($first == 'add_new_category') {
    $data['status'] = 400;
    ...
    //     $data['status'] = 200;
    // }
} else if ($first == 'get_category_langs' && !empty($_POST['lang_key'])) {
    $data['status'] = 400;
...
if ($first == 'add_sub_category') {
...
} else if ($first == 'delete_sub_category') {
    $data['status'] = 400;
...
} else if ($first == 'add_movie_category') {
    $data['status'] = 400;
    $data['message'] = 'Please check your details';
    $add = false;
    $insert_data = array();
...
} else if ($first == 'add_music_category') {
    $data['status'] = 400;
    $data['message'] = 'Please check your details';
    $add = false;
    $insert_data = array();
    ...
    if ($add == true && !empty($insert_data)) {
        $insert_data['type'] = 'music_category';
        $id = $db->insert(T_LANGS,$insert_data);
        $db->where('id',$id)->update(T_LANGS,array('lang_key' => $id));
        $data = array('status' => 200);
    }
} else if ($first == 'add_radio_category') {
    $data['status'] = 400;
    $data['message'] = 'Please check your details';
    $add = false;
    $insert_data = array();
    ...
    if ($add == true && !empty($insert_data)) {
        $insert_data['type'] = 'radio_category';
        $id = $db->insert(T_LANGS,$insert_data);
        $db->where('id',$id)->update(T_LANGS,array('lang_key' => $id));
        $data = array('status' => 200);
    }
...
////////////////
    ...
    //     $insert_data['type'] = 'alboum';
    //     $id = $db->insert(T_LANGS,$insert_data);
    //     $db->where('id',$id)->update(T_LANGS,array('lang_key' => 'alboum__'.$id));
    //     $data['status'] = 200;
    // }
} else if ($first == 'add_new_channel') {
    $data['status'] = 400;
    $data['message'] = 'Please check your details';
    $add = true;
    $insert_data = array(
        ...
        'email' => PT_Secure($_POST['username'].'@uoozet.com', 0),
    ...
    if ($add == true && !empty($insert_data)) {
        $id = $db->insert(T_USERS,$insert_data);
        $data = array('status' => 200);
    }
} else if ($first == 'get_alboum_langs' && !empty($_POST['lang_key'])) {
    $data['status'] = 400;
    $pt->edit_alboum = $db->where('lang_key',PT_Secure($_POST['lang_key']))->getOne(T_LANGS);
    if (!empty($pt->edit_alboum)) {
        $data['html'] = PT_LoadAdminPage('manage_alboums/langs_list');
        $data['status'] = 200;
    }
}
if ($first == 'edit_alboum' && !empty($_POST['lang_key'])) {
...
if ($first == 'delete_alboum' && !empty($_POST['lang_key'])) {
    if ($_POST['lang_key'] != 'other') {
        $db->where('lang_key',PT_Secure($_POST['lang_key']))->delete(T_LANGS);
        $db->where('type',PT_Secure($_POST['lang_key']))->delete(T_LANGS);
        $db->where('alboum_id',PT_Secure($_POST['lang_key']))->update(T_VIDEOS,array('sub_alboum' => '',
            'alboum_id' => 'other'));
        $data['status'] = 200;
    }
}
if ($first == 'add_seoData') {
    $url = $_GET['url'];
    $sd = new SeoData();
    list($page, $cid) = $sd->extractPageInfoFromURL($url);
    $sd->addOne($url, $page, $cid);
}
else if ($first == 'delete_seoData') {
    $id = intval($_GET['id']);
    $sd = new SeoData();
    $sd->delOneById($id);
} else if ($first == 'edit_seoData') {
    $url = $_GET['url'];
    $page = $_GET['page'];
    $cid = intval($_GET['cid']);
    $cid = (($cid === -1)?(NULL):($cid));
    $desc = $_GET['desc'];
    $canonical = explode(",", $_GET['canonical']);
    $sd = new SeoData();
    $sd->setOne($url, $page, $cid, $desc, $canonical);
}
